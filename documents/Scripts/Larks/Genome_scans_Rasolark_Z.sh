...
# Create folders
mkdir $OUTDIR0/E2a_genome_scans_Rasolark_Z;
OUTDIR0=$OUTDIR0/E2a_genome_scans_Rasolark_Z;
...
for DATA in $DATASETS; do
 ...
 --analysis popDist popFreq $PARAMETERS;
  ...
  cat $METADATA/Rasolark_sample_info.txt | tail -n+2 | \
  awk -F'\t' '{print $1"\t"$3}' | tr ' ' '_' \
  > $OUTDIR2/individual_population.txt;

  python3 $genomics_general/popgenWindows.py -g $OUTDIR0/Reuseable_files/Rasolark_$DATA\_$EXON_DIST.geno.gz -T 20 \
  -f phased --writeFailedWindows --ploidyFile $ploidy -o $OUTDIR2/Rasolark_$DATA.pairwise_populations \
