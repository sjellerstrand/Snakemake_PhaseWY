...
# Create folders
mkdir $OUTDIR0/E2a_genome_scans_Skylark_A;
OUTDIR0=$OUTDIR0/E2a_genome_scans_Skylark_A;
...
for DATA in $DATASETS; do
  ...
  if [[ $EXON_DIST =~ ^[0-9]+$ ]]; then
    ...
    bedtools subtract -a $BEDS/autosomal.bed \
    ...
    cat $BEDS/autosomal.bed | awk -F'\t' '{if($1 == "scaffold_1") print}' \
    > $OUTDIR0/Reuseable_files/$DATA\_callable_$EXON_DIST.bed;
  fi;
  ...
  if [ $(find $OUTDIR0 | grep "$OUTDIR0/Reuseable_files/Skylark_$DATA\_$EXON_DIST.geno.gz" | wc -l) == 0 ]; then
    bcftools view $VCFS/autosomal_filtered.vcf.gz -R $MASK | \
    vcfclassify - | \
