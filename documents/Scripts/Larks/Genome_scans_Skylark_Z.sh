...
BEDS=$WORKDIR/Skylark_Europe_sex_depth_0.75_whatshap_ON/mac_1_window_10000_step_2500_model_Hamming/beds;
...
# Create folders
mkdir $OUTDIR0/E2a_genome_scans_Skylark_Z;
OUTDIR0=$OUTDIR0/E2a_genome_scans_Skylark_Z;
...
for DATA in $DATASETS; do
  ...
  if [[ $EXON_DIST =~ ^[0-9]+$ ]]; then
    ...
    bedtools subtract -a $BEDS/homogametic.bed \
    ...
    cat $BEDS/homogametic.bed | awk -F'\t' '{if($1 == "scaffold_1") print}' \
    > $OUTDIR0/Reuseable_files/$DATA\_callable_$EXON_DIST.bed;
  fi;
  ...
  if [ $DATA == "autosomal" ]; then
    ...
    if [ $NONDIPLOIDS == "Yes" ]; then
      ...
      cat $METADATA/Skylark_sample_info.txt | tail -n +2 | awk -F'\t' '{print $1"\t2"}' \
      > $OUTDIR2/ploidy.txt;
    ...
    if [ $SEX_SYSTEM != NA ]; then
      ...
      if [ $DATA == "Z" ]; then
        ...
        cat $METADATA/Skylark_sample_info.txt | awk -F'\t' -v HETGAM=$HETGAM '$3==HETGAM {print $1"\t1"}' \
        > $OUTDIR2/ploidy1.txt;
        cat $METADATA/Skylark_sample_info.txt | awk -F'\t' -v HOMGAM=$HOMGAM '$3==HOMGAM {print $1"\t2"}' \
        > $OUTDIR2/ploidy2.txt;
  ...
  if [ $(find $OUTDIR0 | grep "$OUTDIR0/Reuseable_files/Skylark_$DATA\_$EXON_DIST.geno.gz" | wc -l) == 0 ]; then
    bcftools view $VCFS/homogametic_filtered.vcf.gz -R $MASK | \
    vcfclassify - | \
    ...
    bgzip -c > $OUTDIR0/Reuseable_files/Skylark_$DATA\_snps.vcf.gz;
    python3 $genomics_general/VCF_processing/parseVCF.py -i $OUTDIR0/Reuseable_files/Skylark_$DATA\_snps.vcf.gz \
    --ploidyFile $ploidy | bgzip -c > $OUTDIR0/Reuseable_files/Skylark\_$DATA\_$EXON_DIST.geno.gz;
  fi;
  ...
  python3 $genomics_general/popgenWindows.py -g $OUTDIR0/Reuseable_files/Skylark_$DATA\_$EXON_DIST.geno.gz \
 -f phased --writeFailedWindows --ploidyFile $ploidy -o $OUTDIR2/Skylark_$DATA.pi_tajimas_D \
 --analysis popDist popFreq $PARAMETERS;

  #### Calculate population pairwise Fst and Dxy
  cat $METADATA/Skylark_sample_info.txt | tail -n+2 | \
  awk -F'\t' '{print $1"\t"$3}' | tr ' ' '_' \
  > $OUTDIR2/individual_population.txt;

  python3 $genomics_general/popgenWindows.py -g $OUTDIR0/Reuseable_files/Skylark_$DATA\_$EXON_DIST.geno.gz -T 20 \
  -f phased --writeFailedWindows --ploidyFile $ploidy -o $OUTDIR2/Skylark_$DATA.pairwise_populations \
  --popsFile $OUTDIR2/individual_population.txt -p Female -p Male $PARAMETERS \
  --analysis popDist popFreq popPairDist;
